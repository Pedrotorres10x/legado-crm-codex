...
const AdminAgentEvaluationCard = ({ agentId, data }: Props) => {
    ...
    <Card className="border-0 shadow-card">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          Ficha de evaluación del agente
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className={`rounded-xl border p-4 ${statusTone[evaluation.verdict.tone]}`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm opacity-90">{evaluation.verdict.detail}</p>
            </div>
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          ...
          <div className="rounded-xl border border-border/60 bg-muted/20 p-4">
            ...
            <div className="mt-2">
              <Badge variant="outline" className={autonomyCfg.badge}>
          ...
          <div className="rounded-xl border border-border/60 bg-muted/20 p-4">
            ...
            <p className="mt-2 text-sm font-semibold">
              {influenceCircle?.total ?? 0} contactos utiles
            </p>
        ...
        <div className="rounded-xl border border-border/60 bg-background p-4">
          <p className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Diagnostico en una frase</p>
          <p className="mt-2 text-sm text-muted-foreground">
            {evaluation.weakPillars.length > 0
              ? `Hoy puedes decirle con datos en la mano que ${evaluation.weakPillars.join(', ')}.`
              : 'Hoy puedes defender con datos que el agente está haciendo lo que toca para construir negocio real, aunque la venta llegue más tarde.'}
          </p>
        </div>
      </CardContent>
    </Card>
...
export default AdminAgentEvaluationCard;
