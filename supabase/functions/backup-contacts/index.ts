...
interface BackupRequestBody {
  mode?: string;
  target?: "contacts" | "properties" | "all";
}
...
async function getAdminEmails(supabase: SupabaseClient) {
  ...
  const adminIds = (adminUsers as AdminUserRole[]).map((u) => u.user_id);
  ...
  for (const id of adminIds) {
    ...
    if (user?.email) {
      const profile = (profiles as AdminProfile[] | null)?.find((p) => p.user_id === id);
    ...
    body: JSON.stringify({
      ...
      htmlContent: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:20px;">
          ...
          <p>Se adjuntan los backups completos de contactos e inmuebles no HabiHub.</p>
          ${summary}
...
Deno.serve(async (req) => {
  try {
    ...
    const body = req.method === "POST"
      ? await req.json().catch(() => ({} as BackupRequestBody))
      : {} as BackupRequestBody;
    const mode = body.mode || "download";
    const target = body.target || "all"; // "contacts", "properties", "all"
    ...
    if (target === "properties" || target === "all") {
      ...
      const properties = await fetchAllRows(supabase, "properties", propCols, "source.is.null,source.neq.habihub");
      if (properties.length > 0) {
        const csv = buildCsv(properties, propCols.split(", "));
        const fileName = `backup-inmuebles-no-habihub-${dateStr}.csv`;
        await supabase.storage.from("contact-backups").upload(fileName, csv, { contentType: "text/csv", upsert: true });
    ...
    const summaryRows = Object.entries(results).map(([key, r]) =>
      `<tr><td style="padding:4px 12px;color:#666;">${key === "contacts" ? "Contactos" : "Inmuebles no HabiHub"}</td><td style="padding:4px 12px;font-weight:bold;">${r.total.toLocaleString("es-ES")}</td><td style="padding:4px 12px;">${r.fileName}</td></tr>`
    ).join("");
    ...
    // For manual download: if single target, return that CSV; if all, return contacts CSV.
